...
$(document).ready(function () {
    ...
    wow.init();
    $(".nav-links__item").on("click", "a", function (event) {
    ...
    $('.main-slider').slick({
        prevArrow: '<span class="icon-arrow-left main-slider_prev main-slider__arrows"></span>',
        nextArrow: '<span class="icon-arrow-right main-slider_next main-slider__arrows"></span>',
        draggable: false,
        autoplay: true,
        autoplaySpeed: 10000,
        infinite: true,
    ...
    function sliderCount(element) {
        $(element).eq($('header .slick-track .slick-active').data('slick-index')).addClass('active').siblings().removeClass('active');
    }
    ...
    function setCoordinates() {
        ...
        number_to(latitude, parseInt(latitude.innerText, 16), parseInt(coordinate.left, 16), 2000);
        number_to(longitude, parseInt(longitude.innerText, 16), parseInt(coordinate.top, 16), 2000);
    }
    setCoordinates();
    ...
    function setTravelOptions() {
        ...
        number_to(miles, parseNumber(miles.innerText), parseNumber(sliderInfo.miles.toString()), 2000);
        number_to(hours, parseNumber(hours.innerText), parseNumber(sliderInfo.hours.toString()), 2000);
        number_to(minutes, parseNumber(minutes.innerText), parseNumber(sliderInfo.minutes.toString()), 2000);
        number_to(price, parseNumber(price.innerText), parseNumber(sliderInfo.price.toString()), 2000);
    }
    setTravelOptions();
    $('.travel .travel-slider').on('afterChange', function() {setTravelOptions()});
    ...
    function scoreSleepOptions () {
        let slide = $('.sleep .travel-slider .slick-current'),
            ...
            total = +nights.text() *  +guests.text() * sliderInfo.nightsIncrease;
            number_to(price, parseNumber(price.innerText), parseNumber(total.toString()), 2000);
    }
    $('.count-button').click(function () {
        let countEl = $(this).closest('.table__title').find('.count'),
            count = +countEl.html();

        ($(this).hasClass('plus-minus__minus')) ? countEl.html(--count) : countEl.html(++count);
        scoreSleepOptions();
    })

    setSleepOptions();
    $('.sleep .travel-slider').on('afterChange', function() {setSleepOptions()});
